<template>
  ...
  >
    ...
    >
      ...
      <div fc lg:flex-row lg:gap-4>
        <HInput
          flex-1
          :rules="v.required"
          v-model="sender.yield"
        ...
        <HInput
          flex-1
          :rules="v.required"
          v-model="sender.liquidityGen"
          label="Transaction fee to generate liquidty (%)"
          :bgColor="theme === 'dark' ? 'bg-primaryBg' : ''"
        />
      </div>
      ...
      <HInput
        :rules="v.required"
        v-model="sender.mktw"
        label="Marketing reciever (address)"
        :bgColor="theme === 'dark' ? 'bg-primaryBg' : ''"
      />

      <HInput
        :rules="v.required"
        v-model="sender.mktfee"
        label="Marketing fee (%)"
        :bgColor="theme === 'dark' ? 'bg-primaryBg' : ''"
      />
